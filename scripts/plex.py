#source: https://github.com/artic-network/fieldbioinformatics/blob/master/artic/guppyplex.py
import sys
import os
import gzip
import fnmatch
...
def main():
    ...
    parser.add_argument("--directory", metavar="directory", required=True,
                         help="Basecalled and demultiplexed (guppy) results directory")
    parser.add_argument("--max-length", dest="max_length", type=int, metavar="max_length",
                         help="remove reads greater than read length")
    parser.add_argument("--min-length", dest="min_length", type=int, metavar="min_length",
                         help="remove reads less than read length")
    parser.add_argument("--quality", type=float, metavar="quality", default=7,
                         help="remove reads against this quality filter")
    parser.add_argument("--sample", type=float, metavar="sample", default=1,
                         help="sampling frequency for random sample of sequence to reduce excess")
    parser.add_argument("--skip-quality-check", dest="skip_quality_check", action="store_true",
                         help="Do not filter on quality score (speeds up)")
    parser.add_argument("--prefix", help="Prefix for guppyplex files")
    parser.add_argument("--output", metavar="output", help="FASTQ file to write")
    parser.add_argument("--threads", type=int, metavar="threads", default=1,
